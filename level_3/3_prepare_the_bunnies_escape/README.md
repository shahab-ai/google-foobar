...
the door to an escape pod. The map is represented as a matrix of 0s and 1s,
where 0s are passable space and 1s are impassable walls. The door out of the
prison is at the top left (0,0) and the door into an escape pod is at the bottom
right (w-1, h-1).
...
from 2 to 20. Move can only be made in cardinal directions; no diagonal moves
are allowed.
...
To provide a Python solution, edit solution.java
